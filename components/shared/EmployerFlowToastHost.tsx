...
import { EMPLOYER_FLOW_TOAST_EVENT, type EmployerFlowToastDetail } from "@/lib/employer-flow";

export function EmployerFlowToastHost() {
  ...
  useEffect(() => {
    function onToast(e: Event) {
      const ce = e as CustomEvent<EmployerFlowToastDetail>;
      if (ce.detail?.message) setMessage(ce.detail.message);
    }
    window.addEventListener(EMPLOYER_FLOW_TOAST_EVENT, onToast as EventListener);
    return () => window.removeEventListener(EMPLOYER_FLOW_TOAST_EVENT, onToast as EventListener);
  }, []);
