...
> **Статус миграции (Волна 2 / эпик E0 — завершён 2026-06-22):** обе роли (подросток и работодатель) полностью переведены на Supabase — каталог, отклики, избранное, профили, задачи, управление откликами/статусами, создание/редактирование задач. Закрыта и F0.6 — онбординг подростка при первом входе (`app/teen/onboarding`, гейт по `onboarded`), цель заработка перенесена из localStorage в Supabase (`teen_profiles.earning_goal_amount`). Часть демо-логики (`lib/*-flow.ts`, `lib/*-storage.ts`, `data/demo-*`, мок-сессия в `lib/auth.ts`) ещё присутствует как переходный слой и постепенно выводится.
...
- **Сценарий подростка**
  - `app/teen/onboarding/page.tsx`: онбординг при первом входе (F0.6); серверный гейт по `onboarded`, UI — `components/teen/onboarding/OnboardingFlow.tsx` (цель → мотивация → формат → выходные → интересы). Поля затем видны и редактируются в профиле.
...
- **Переходный слой:** `data/demo-*`, `lib/*-flow.ts`, `lib/*-storage.ts` и мок-сессия в `lib/auth.ts` ещё используются точечно (часть лендинга/хелперов) и выводятся по мере доработок. Цель заработка подростка перенесена из localStorage в Supabase (`lib/teen-earning-goal.ts` → `teen_profiles.earning_goal_amount`), что закрывает остаток F16.2 по хранению; визуализация «до цели осталось» — отдельный шаг E16.
